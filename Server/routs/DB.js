...
function fExecuteSP(EXECParams,SPName){
    return new Promise((resolve,reject)=>{
        ...
        pool.connect(function (err){ //Try to connect
            ...
            {
                req.parameters=EXECParams.parameters; //Get parameters

                req.execute(SPName,(err,recordset)=>{ //commit query
                    if (err){
                        console.log(err);
                    }
                    else{
...
module.exports.ExecuteSP=fExecuteSP;